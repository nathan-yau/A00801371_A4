...
def map_widget_update(avatar: dict, overall_frames: dict, game_map_path: str = GAME_MAP_PATH) -> None:
    """
    ...
    :param game_map_path: a file path referring to the game map image
    ...
    :precondition: game_map_path must be a file path referring to the game map image
    ...
    if pathlib.Path(game_map_path.format(coordinate)).is_file():
        update_image_label(frame_object=overall_frames['Status Frame'], label_name="current_map",
                           image_directory=GAME_MAP_PATH.format(coordinate))
